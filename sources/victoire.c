...
int detailsVictoirePoker(SDL_Surface *screen, JoueurPoker *t, CentrePlateau cp, int nbJoueurs, int winner)
{
  ...
  while (continuer == 1)
  {
    ...
    {
      if (t[i].etat == 1)
        SDL_BlitSurface(etatDead, NULL, screen, &posEtat[i]);
      else if (t[i].joueur == winner)
        SDL_BlitSurface(etatWin, NULL, screen, &posEtat[i]);
      else
        SDL_BlitSurface(etatLoose, NULL, screen, &posEtat[i]);
    }
