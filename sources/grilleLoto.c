...
void carteLoto (){
  ...
  int t[9][3]; //grille loto
  for (i=0; i<9; i++){  //initialisation de la grille a -1 (vide)
    for (j=0; j<3; j++){
      t[i][j]=-1;
    }
  }
  while (n!=15){
    ...
    if(t[i][j]==-1 && nbNombresLigne<15){ //alors on met un chiffre dans la grille
